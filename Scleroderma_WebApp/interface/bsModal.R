...
shinyBSDep <- htmltools::htmlDependency("shinyBS", packageVersion("shinyBS"), src = c("href" = "sbs"), script = "shinyBS.js")
...
bsModal <- function(id, title, trigger, ..., size, footer = NULL, close.button = TRUE, width = NULL) {
  ...
  #shinyBSDep <- htmltools::htmlDependency(script = "shinyBS.js")
  htmltools::attachDependencies(bsTag, shinyBSDep)
  
}
bsModal_init <- function(id, title, trigger, ..., size, footer = NULL, close.button = TRUE, width = NULL) {
  if(!missing(size)) {
    if(size == "large") {
      size = "modal-lg"
    } else if(size == "small") {
  ...
  bsTag <- shiny::tags$div(class = size, 
                           shiny::tags$div(class = "modal fade in", 
                                           shiny::tags$div(class = "modal-header",
                                                           shiny::tags$button(type = "button", class = "close", "data-dismiss" = "modal", shiny::tags$span(shiny::HTML("&times;"))),
                                                           shiny::tags$h4(class = "modal-title", title)
                                           ),
